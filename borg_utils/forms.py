...
class BorgModelForm(django.forms.ModelForm):
    ...
    def get_mode(self,data):
        """
        return a tuple(mode,hook,check required?,full_clean?)
        """
        if data and any(key in data for key in ["_save","_continue","_saveasnew","_addanother"]):
            return (BorgModelForm.SAVE,None,True,True)
        elif data and "_validate" in data:
            return (BorgModelForm.VALIDATE,None,True,True)
        else:
            return (BorgModelForm.EDIT,"edit",False,False)
    ...
    def _clean_fields(self):
        for name, field in self.fields.items():
            ...
            try:
                if not self._mode[2] and (not value or not value.strip()):
    ...
    def _post_clean(self):
        ...
        if self._mode[0] == BorgModelForm.SAVE and self.instance.pk:
            ...
            for field in self.base_fields.keys():
                if self.cleaned_data[field] != getattr(self.instance,field):
